...
bore_data <- bore_data %>%
  select(bore_id, bore_no, easting, northing, zone_id, bore_depth, site_id, primary_use) # take location from the bore, "aquifer" also available

nitrate_data <- lab_data %>%
  filter((test_name_group == "Nutrients" & (test_name %in% c("Total Nitrogen", "Nitrate-N + Nitrite-N"))) | test_name == "Nitrate-N") %>%
