...
trend_var_name <- gsub("\\[[^\\]]*\\]", "", trend_var, perl=TRUE) %>% str_sub(start = 1, end = -2) # drop units from trend_var (overly complicated way)
...
analysis_data_qtly <- analysis_data %>% # use median value per quarter
  group_by(site_for_analysis, year_quarter, catchment) %>%
...
trend_analysis <- analysis_data_qtly %>%
  group_by(site_for_analysis, catchment) %>%
  filter(n() > 4) %>%
  summarise(
    variable = trend_var,
    n_records = n(),
  ...
  select(site_for_analysis, catchment, variable, n_records, mann_kendall_tau, mann_kendall_sl) %>%
  mutate(
...
st_write(trend_analysis, glue("trend-analysis/outputs/gwq_gns_trend_results_{trend_var_name}.gpkg"), "gwq_gns_observations", append = FALSE)
write_csv(trend_analysis, glue("trend-analysis/outputs/gwq_gns_trend_results_{trend_var_name}.csv"))
...
plot_trend_by_site <- function(site, analysis_data, analysis_data_qtly) {
  ...
  ggsave(glue("trend-analysis/outputs/plots/{site}_{trend_var_name}_trend_plot.png"), plot = plot_site, width = 10, height = 7)
}
...
#   n <- n_pages(gg)
#   
#   pdf(glue("trend-analysis/outputs/{catchment}_{trend_var_name}_Summary.pdf"), paper= 'A4', w= 210/20, 297/20)
#   for(i in 1:n){
